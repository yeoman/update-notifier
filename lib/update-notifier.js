...
function UpdateNotifier(options) {
	...
	this.updateCheckTimeout = options.updateCheckTimeout || 10000;                 // 10 secs
...
if (process.env._UPDATE_NOTIFIER_OPTIONS) {
	...
	updateNotifier.checkNpm(function(update) {
		if (update.type && update.type !== 'latest') {
			updateNotifier.config.set('update', update);
		}

		// Call process exit explicitly to terminate the child process
		// Otherwise the process will run forever (according to nodejs docs)
		process.exit();
	});
} else {
